...
class Login extends Component {
  ...
  handleSubmit = (event) => {
    ...
    const hashedPassword = md5('Jiyan');
      ...
      <div style={styles.container}>
        <form onSubmit={this.handleSubmit} style={styles.form}>
          <label style={styles.label}>
            Password:
            <input type="password" value={this.state.password} onChange={this.handlePasswordChange} style={styles.input} />
          </label>
          <button type="submit" style={styles.button}>Login</button>
          {this.state.errorMessage && <p style={styles.error}>{this.state.errorMessage}</p>}
        </form>
      </div>
    );
  };
